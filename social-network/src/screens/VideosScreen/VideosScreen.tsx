...
const VideosScreen = () => {
  ...
  const {
    ...
    error: postsError,
  } = useGetPostsQuery({ page: 1, page_size: 10 }, { refetchOnMountOrArgChange: true });

  const videoPosts = postsData?.data?.filter((post) => post.type === 'videos') || [];
        ...
        {searchString && (
          <div>
            <p className='text-[15px] font-semibold'>
              Main <span className='opacity-50'>/ Videos</span>
            </p>
