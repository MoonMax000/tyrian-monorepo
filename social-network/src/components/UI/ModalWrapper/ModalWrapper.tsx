...
import IconCrossCirlce from '@/assets/icons/icon-close.svg';
...
const ModalWrapper: FC<ModalWrapperProps> = ({
  ...
  return ReactDOM.createPortal(
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className='fixed w-full z-[1001] backdrop-blur-md h-full flex justify-center items-center'
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={(e) => {
        ...
        >
          ...
          >
            {onClose && withCloseIcon && (
              <button
                type='button'
                className='absolute right-6 top-8 size-6 z-30'
                onClick={onClose}
              >
                <IconCrossCirlce />
              </button>
