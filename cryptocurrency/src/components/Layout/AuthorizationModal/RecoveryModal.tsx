import Button from '@/components/UI/Button';
import Input from '@/components/UI/Input';

import { FC, useState } from 'react';
import { useMutation } from '@tanstack/react-query';
import { AuthorizationService } from '@/services/AuthorizationService';
import { TAuthorizationModal } from '@/types/authorization';
import PostIcon from '@/assets/icons/input/post.svg';

interface FormState {
  email: string;
  password: string;
  confimPaswword: string;
}

type ModalSteps = 'sendMassage' | 'successNotification';

const RecoveryModal: FC<{
  onModalChange: (type: TAuthorizationModal) => void;
}> = ({ onModalChange }) => {
  const [formState, setFormState] = useState<FormState>({} as FormState);
  const [step, setStep] = useState<ModalSteps>('sendMassage');

  const { mutateAsync, isPending } = useMutation({
    mutationKey: ['SendMessage'],
    mutationFn: async () => {
      const loginBody = {
        email: formState.email,
      };

      try {
        const data = await AuthorizationService.requestResetPassword(loginBody);
        console.log(data);
        switch (data.status) {
          case 204:
            setStep('successNotification');
            break;
          case 400:
            alert('Введите правильный адрес электронной почты.');
            break;
          default:
            alert('Введите правильный адрес электронной почты.');
            break;
        }
      } catch (error) {
        console.log('Login failed:', error);
      }
    },
    onSuccess: (data) => {
      console.log(data);
    },
    onError: (error) => {
      console.log('invalid CurrentPassword', error);
    },
  });
  const handleSendMessage = async () => {
    mutateAsync();
  };

  const stepData = {
    sendMassage: {
      notification: 'Введите адрес электронной почты для восстановления пароля.',
      disabled: !formState.email,
      buttonValue: 'Отправить письмо',
      buttonAction: () => handleSendMessage(),
    },
    successNotification: {
      notification:
        'Письмо со ссылкой на форму сброса пароля было отправлено на вашу электронную почту.',
      disabled: false,
      buttonValue: 'Продолжить',
      buttonAction: () => onModalChange(null),
    },
  };

  return (
    <>
      <p className='mb-6 text-basic opacity-[48%]'>{stepData[step].notification}</p>
      <div>
        <div className='flex flex-col gap-4 mb-6'>
          {step === 'sendMassage' && (
            <Input
              beforeIcon={
                <div className='opacity-45 mr-2'>
                  <PostIcon width={20} height={20} className='text-white ' />
                </div>
              }
              placeholder='E-mail'
              inputWrapperClassName={'min-h-11 border-0'}
              onChange={(e) => setFormState((prev) => ({ ...prev, email: e.target.value }))}
              value={formState.email}
            />
          )}
        </div>
        <Button
          className='mx-auto max-w-[180px] min-h-10 w-full'
          type='submit'
          disabled={stepData[step].disabled || isPending}
          onClick={stepData[step].buttonAction}
        >
          {stepData[step].buttonValue}
        </Button>
      </div>
    </>
  );
};

export default RecoveryModal;
