...
const CurrencyStocksBlock = () => {
  return (
    <>
      <h4 className='text-h4 pl-6'>Инвесторы также отслеживают</h4>
        ...
        {mockStocks.map((stock) => (
          <Paper key={stock.shortName} className='!bg-blackedGray !px-6 !flex flex-col gap-6'>
            <div className='flex items-center gap-2'>
              <p className='text-basic'>{stock.name}</p>
              <p className='text-body-12 opacity-[48%] uppercase'>{stock.shortName}</p>
            </div>
