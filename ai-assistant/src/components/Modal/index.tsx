...
import IconCrossCirlce from '@/assets/modal/icon-cross.svg';
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <div>
              {title && (
                <h4 className={clsx('h4 mb-8', titleClassName)}>{title}</h4>
              )}

              {label && (
                <p className='text-body-15 opacity-[48%] whitespace-pre-line'>
                  {label}
                </p>
              )}
              {labelValue && (
                <p className='text-body-15 whitespace-pre-line'>{labelValue}</p>
              )}
            </div>

            <div
              className={clsx(
                'max-h-full overflow-auto pr-2 scrollbar',
                contentClassName,
              )}
            >
              {children}
