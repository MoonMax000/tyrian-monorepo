...
func printLogo() {
	fmt.Println()
	fmt.Printf("STREAMS NOTIFY SERVICE V1.0")
	fmt.Println()
}
...
func processQueue(cfg *config.Config) {
	...
	// Email sender
	// mailQueueConnector := &MailQueueConnector{
	// 	cfg: cfg,
	// }
	...
	go func() {
		for {
			...
			go func(message *amqp.Delivery) {
				...
				redis_notifications.SaveNotification(notifyMessage.Type, notifyMessage.UserId, notifyMessage.NotifyTimestamp, notifyMessage.MessageParams, notifyMessage.TargetUserId)
				// if notifyMessage.UserInfo != nil {
				// 	log.Info().Msgf("Message %v will send to notify user: %q", notifyMessage.Type, notifyMessage.UserInfo.UserId)
				// }
				// err = mailQueueConnector.PushEmail(&notifyMessage)
				// if err != nil {
				// 	log.Error().Msgf("Can't push message to mail queue cause error: %v", err)
				// 	return
				// }

				// if notifyMessage.UserInfo != nil {
				// 	log.Info().Msgf("Message %v sent to notify user %q", notifyMessage.Type, notifyMessage.UserInfo.UserId)
				// }
