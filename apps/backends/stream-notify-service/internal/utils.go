...
func IsAllowed(notificationType string, filter *model.FilterNotification) bool {
	// Логика фильтров отключена, всегда разрешены, для включения расскоментируй проверку
	return true
	// switch notificationType {
	// case types.SUBSCRIPTION:
	// 	return filter.Subscription
	// case types.STATUSSTREAM:
	// 	return filter.StatusStream
	// default:
	// 	return false // Неизвестный тип — не разрешаем
	// }
}
