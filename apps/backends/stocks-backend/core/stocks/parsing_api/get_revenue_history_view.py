...
def parse_revenue_history(country: str, params) -> dict:
    url = f"{FORMATTER_BASE_URL}/revenue-history/{country}/"
...
class RevenueHistorySerializer(serializers.Serializer):
    ...
    grossProfit = serializers.FloatField(required=True)
    reportedCurrency = serializers.CharField(required=True)


class RevenueHistoryView(generics.GenericAPIView):
    """
    Эндпойнт для получения истории выручки и дохода.
    """
    ...
    @swagger_auto_schema(
        operation_description="Получение истории выручки и дохода.",
        manual_parameters=[
            openapi.Parameter(
                'limit',
                openapi.IN_QUERY,
        ...
        responses={
            200: openapi.Response(
                ...
                schema=RevenueHistorySerializer(many=True),
            ),
    ...
    def get(self, request, country, ticker):
        ...
        data = parse_revenue_history(country.lower(), params)
