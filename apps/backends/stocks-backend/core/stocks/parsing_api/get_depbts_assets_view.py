...
def parse_depbts_assets(country: str, params: dict) -> dict:
    url = f"{FORMATTER_BASE_URL}/depbts-assets/{country}/"
    data = http_client.get(url, params=params)
    return data


class DepbtsAssetsSerializer(serializers.Serializer):
    date = serializers.CharField(required=True)
    totalAssets = serializers.FloatField(required=True)
    totalLiabilities = serializers.FloatField(required=True)
    totalDebt = serializers.FloatField(required=True)
    netDebt = serializers.FloatField(required=True)
    reportedCurrency = serializers.CharField(required=True)


class DepbtsAssetsView(generics.GenericAPIView):
    """
    Эндпойнт для получения выручки и чистой прибыли.
    """

    permission_classes = [permissions.IsAuthenticated]
    serializer_class = DepbtsAssetsSerializer

    @swagger_auto_schema(
        operation_description="Получение долгов и активов.",
        manual_parameters=[
            openapi.Parameter(
                'limit',
                openapi.IN_QUERY,
        ...
        responses={
            200: openapi.Response(
                description="Успешный ответ с данными о долгах и активах.",
                schema=DepbtsAssetsSerializer(many=True),
            ),
    ...
    def get(self, request, country, ticker):
        ...
        data = parse_depbts_assets(country.lower(), params)
