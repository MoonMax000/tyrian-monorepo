...
class ETFKind(Enum):
    ...
    highest_aum_growth = "highest aum growth"
    highest_returns = "highest returns"
    highest_dividend_yields = "highest dividend yields"
    equity_etf = "equity etf"
    commodities_etf = "commodities etf"
    bitcoins_etf = "bitcoins etf"
    gold_etfs = "gold etfs"


KIND_CHOICES = [item.value for item in ETFKind]


class ETFSortBy(Enum):
    name = "name"
    price = "price"
    price_change = "price_change"
    total_return_on_net_assets = "total_return_on_net_assets"

SORT_BY_CHOICES = [item.value for item in ETFSortBy]

def parse_etf(params: dict, *kwargs) -> dict:
    logger.info(f"parse_emissions_corporate called with params: {params}")
    url = f"{FORMATTER_BASE_URL}/etf/general/"
...
class ETFSerializer(serializers.Serializer):
    data = serializers.JSONField(required=True)

class ETFView(APIView):
    """
    ...
    @swagger_auto_schema(
        operation_description="Получение ETF.",
        manual_parameters=[
            openapi.Parameter(
                'limit',
                openapi.IN_QUERY,
            ...
            openapi.Parameter(
                'kind',
                openapi.IN_QUERY,
                description="Вид ETF. Допустимые значения: " + ", ".join(KIND_CHOICES),
            ...
            openapi.Parameter(
                'sort_by',
                openapi.IN_QUERY,
                description="Поле для сортировки. Допустимые значения: " + ", ".join(SORT_BY_CHOICES),
                type=openapi.TYPE_STRING,
                enum=SORT_BY_CHOICES,
                required=False
            ),
            openapi.Parameter(
                'sort_dir',
        ...
        responses={
            200: openapi.Response(
                description="Успешный ответ с данными о ETF.",
                schema=ETFSerializer(),
            ),
    ...
    def get(self, request):
        logger.info(f"ETFView.get called with query_params: {request.query_params}")
        limit = request.query_params.get("limit", 25)
        page = request.query_params.get("page", 1)
        kind = request.query_params.get("kind", ETFKind.all_etfs)
        sort_by = request.query_params.get("sort_by", ETFSortBy.price_change)
        ...
        if sort_by is not None and sort_by not in SORT_BY_CHOICES:
            logger.warning(f"Invalid sort_by value: {sort_by}")
            return Response({
                "error": "Недопустимое значение sort_by. Допустимые значения: " + ", ".join(SORT_BY_CHOICES)
            }, status=400)
        ...
        logger.info(f"Calling parse_etf with params: {params}")

        try:
            data = parse_etf(params)
            logger.info(f"Successfully received data from formatter")
        except Exception as e:
            logger.error(f"Exception in parse_etf: {str(e)}", exc_info=True)
