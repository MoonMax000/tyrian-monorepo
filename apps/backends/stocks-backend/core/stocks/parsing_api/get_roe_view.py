...
def parse_roe(country: str, params: dict) -> dict:
    url = f"{FORMATTER_BASE_URL}/roe/{country}/"
...
class ROESerializer(serializers.Serializer):
    ...
    roe = serializers.FloatField(required=True)

class ROEView(generics.GenericAPIView):
    """
    Эндпойнт для получения ROE.
    """

    permission_classes = [permissions.IsAuthenticated]
    serializer_class = ROESerializer

    @swagger_auto_schema(
        operation_description="Получение ROE.",
        manual_parameters=[
            openapi.Parameter(
                'limit',
                openapi.IN_QUERY,
        ...
        responses={
            200: openapi.Response(
                description="Успешный ответ с данными о ROE.",
                schema=ROESerializer(many=True),
            ),
    ...
    def get(self, request, country, ticker):
        ...
        data = parse_roe(country.lower(), params)
