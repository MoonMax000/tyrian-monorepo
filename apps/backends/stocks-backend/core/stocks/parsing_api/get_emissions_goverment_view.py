...
def parse_emissions(params: dict, *kwargs) -> dict:
    logger.info(f"parse_emissions called with params: {params}")
    url = f"{FORMATTER_BASE_URL}/emissions-goverment/general/"
...
class EmissionsGovermentView(APIView):
    """
    Эндпойнт для получения правительственных облигаций.
    """
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter(
                'kind',
                openapi.IN_QUERY,
                description="Вид правительственной облигации. Допустимые значения: " + ", ".join(KIND_CHOICES),
    ...
    def get(self, request):
        logger.info(f"EmissionsGovermentView.get called with query_params: {request.query_params}")
        ...
        logger.info(f"Calling parse_emissions with params: {params}")

        try:
            data = parse_emissions(params)
            logger.info(f"Successfully received data from formatter")
        except Exception as e:
            logger.error(f"Exception in parse_emissions: {str(e)}", exc_info=True)
