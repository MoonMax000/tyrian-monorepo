...
import (
	...
	"github.com/Capstane/stream-recommend-service/internal/handler"
	"github.com/Capstane/stream-recommend-service/internal/router"
...
func httpService(cfg *config.Config) error {
	...
	streamService, err := NewAmqService(
		db,
		cfg,
	)
	if err != nil {
		log.Error().Msgf("Failed to initialize stream service: %v", err)
		return err
	}
	...
	handlers := handler.NewHandler(db, cfg, streamService)
