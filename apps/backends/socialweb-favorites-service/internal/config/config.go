...
import (
	...
	"github.com/Capstane/AXA-socialweb-favorites/internal/utils"
...
type Config struct {
	...
	HttpPort    uint16   `default:"8006" envconfig:"HTTP_PORT"`
	...
	S3ContentBucketName string `binding:"required" envconfig:"S3_CONTENT_BUCKET_NAME"`
...
func LoadConfig() Config {
	...
	return Config{
		LogLevel:    logLevel,
		HttpPort:    parser.ParseUint16(os.Getenv("HTTP_PORT"), 8006),
		...
		PostgresMaxIdleConns:    parser.ParseInt(os.Getenv("POSTGRES_MAX_IDLE_CONNS"), 10),
		PostgresMaxOpenConns:    parser.ParseInt(os.Getenv("POSTGRES_MAX_OPEN_CONNS"), 100),
		PostgresConnMaxLifetime: parser.ParseDuration(os.Getenv("POSTGRES_CONN_MAX_LIFETIME"), 1*time.Hour),
		...
		RedisDB:   parser.ParseInt(os.Getenv("REDIS_DB"), 0),
		...
		S3ContentBucketName: os.Getenv("S3_CONTENT_BUCKET_NAME"),

		MaxFileUploadSizeInBytes: parser.ParseInt(os.Getenv("MAX_FILE_UPLOAD_SIZE"), 10485760),

		RMQConnUrl:            os.Getenv("RMQ_CONN_URL"),
		RMQConsumeQ:           os.Getenv("RMQ_QUEUE_CONSUME"),
		RMQConsumeB:           utils.Getenv("RMQ_BINDING_CONSUME", "notify.*"),
		RMQExchange:           utils.Getenv("RMQ_EXCHANGE", "notify"),
		RMQConsumeQAutocreate: !parser.ParseBool(os.Getenv("RMQ_QUEUE_CONSUME_AUTOCREATE_DISABLED")),
	}
}
