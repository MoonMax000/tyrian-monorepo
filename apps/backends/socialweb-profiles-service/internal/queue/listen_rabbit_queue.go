...
import (
	...
	"github.com/Capstane/AXA-socialweb-profile/internal/dto"
...
func printLogo() {
	fmt.Println()
	fmt.Printf("SOCIALWEB PRODILES SERVICE V1.0")
	fmt.Println()
}
...
func processQueue(cfg *config.Config) {
	...
	go func() {
		for {
			message := <-chProfileSenders
			log.Debug().Msgf("New message with timestamp %v", message.Timestamp)

			go func(message *amqp.Delivery) {
				var timestampMessage dto.TimestampMessage

				err := json.Unmarshal(message.Body, &timestampMessage)
				if err != nil {
					log.Error().Msgf("Can't unmarshal body %v...: %v", cutBytes(message.Body, 100), err)
					return
				}

				log.Info().Msgf("Message: UserId: %v, LastTimestamp: %v", timestampMessage.UserId, timestampMessage.LastTimestamp)
...
func getMessagesChannel(conn *amqp.Connection, cfg *config.Config) (<-chan amqp.Delivery, error) {
	...
	if err != nil && cfg.RMQConsumeQAutocreate {
		...
		err = autocreateQueue(conn, cfg.RMQConsumeQ)
		if err != nil {
			return nil, err
		}
		ch, err = conn.Channel()
